...
function contentLength(value) {
    return hasContent(value) ? String(value).length : 0;
}

function summarizeContentArray(values) {
    const items = Array.isArray(values) ? values : [];
    return {
        count: items.length,
        contentLength: items.reduce((total, value) => total + contentLength(value?.content ?? value), 0),
    };
}

export function summarizeWorldInfoResult(result) {
    const source = result && typeof result === "object" ? result : {};
    const depthEntries = Array.isArray(source.WIDepthEntries) ? source.WIDepthEntries : [];
    const outletValues = source.outletEntries && typeof source.outletEntries === "object"
        ? Object.values(source.outletEntries).filter(Array.isArray)
        : [];
    return {
        worldInfoBefore: { present: hasContent(source.worldInfoBefore), contentLength: contentLength(source.worldInfoBefore) },
        worldInfoAfter: { present: hasContent(source.worldInfoAfter), contentLength: contentLength(source.worldInfoAfter) },
        exampleEntries: summarizeContentArray(source.EMEntries),
        authorsNoteBeforeEntries: summarizeContentArray(source.ANBeforeEntries),
        authorsNoteAfterEntries: summarizeContentArray(source.ANAfterEntries),
        depthEntries: {
            groupCount: depthEntries.length,
            count: depthEntries.reduce((total, entry) => total + (Array.isArray(entry?.entries) ? entry.entries.length : 0), 0),
            contentLength: depthEntries.reduce((total, entry) => total + summarizeContentArray(entry?.entries).contentLength, 0),
        },
        outletEntries: {
            outletCount: outletValues.length,
            count: outletValues.reduce((total, entries) => total + entries.length, 0),
            contentLength: outletValues.reduce((total, entries) => total + summarizeContentArray(entries).contentLength, 0),
        },
    };
}
...
export async function resolveWorldInfoContext({
    ...
    const resolved = {
        records,
        text: formatWorldInfoRecords(records),
        resultSummary: summarizeWorldInfoResult(result),
    };
    if (includeRawResult) resolved.rawResult = result;
    return resolved;
}
